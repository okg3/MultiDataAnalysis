...
CheckXClass <- function(x){
  if (class(x) == "list"){
    for (i in 1:length(x)){
      if (class(x[[i]]) != "matrix"){
        x[[i]] <- as.matrix(x[[i]])
        warning(paste0("Coercing list item ", i,
      ...
      if(is.null(colnames(x[[i]]))){
        stop(paste0("column names missing from list element ", i, "of x"))
      }
    }
  } else {
    if (class(x) != "matrix"){
      warning("Coercing x to matrix: some data may be lost")
      x <- as.matrix(x)
    }
    ...
    if(is.null(colnames(x))){
      stop("column names missing from x")
    }
    x <- list(x)
  }
  x
