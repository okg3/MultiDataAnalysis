...
CheckFormula <- function(formula, y = NULL, groups = NULL, 
  ...
  for (i in 1:length(xName)){
    if (!(xName[i] %in% formula.vars)) {
      warning(paste0("variable", xName[i], "missing from formula."))
    }
  }
  ...
  if (yName == "auto"){
    ...
    if (yName != formula.vars[1]){
      stop(paste0("yName, ", yName, ", does not match formula"))
    }
  }
  ...
  if (length(modelCov[!(modelCov %in% varsAvail)]) > 0){
    stop(
      paste0("Unable to find the following Vars: ",
             modelCov[!(modelCov %in% varsAvail)]))
  }
